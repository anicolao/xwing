...
export interface ShipState { id: string; seat: Seat; pose: Pose; hull: number; shields: number; force: number; stress: number; focus: number; evade: number; lock?: string; damage: { id: string; faceup: boolean; title: string }[]; maneuver?: Maneuver; revealed: boolean; activated: boolean; engaged: boolean; destroyed: boolean }
export interface GameState {
  ...
  attack?: { attackerId: string; defenderId: string; attack: AttackFace[]; defense: DefenseFace[] };
  winner?: Seat | 'draw'; log: string[];
}
