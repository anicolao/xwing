...
export function rollDice<T>(seed: number, count: number, faces: readonly T[]): { seed: number; results: T[] } {
  const results: T[] = []; let currentSeed = seed;
  for (let index = 0; index < count; index += 1) { const random = nextRandom(currentSeed); currentSeed = random.seed; results.push(faces[Math.floor(random.value * faces.length)]!); }
