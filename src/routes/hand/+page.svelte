...
<style>
  ...
  .dials { display:grid; gap:14px; width:100%; } fieldset { margin:0; padding:10px; border:1px solid #6fd4e855; border-radius:8px; } legend { padding:0 8px; font-weight:700; } .maneuvers { display:grid; grid-template-columns:repeat(5,1fr); gap:6px; } .maneuvers button { position:relative; display:grid; grid-template-columns:auto 1fr; align-items:center; width:auto; min-width:0; min-height:48px; margin:0; padding:3px 5px; border:2px solid #d7d9cf; background:#182833; } .maneuvers button.difficulty-blue { border-color:#64cce3; } .maneuvers button.difficulty-red { border-color:#e96f5f; } .maneuvers button.selected { background:#a85e1e; box-shadow:0 0 0 3px #fff; } .maneuvers span { font:700 .92rem 'Space Mono'; } .maneuvers img { width:100%; height:34px; object-fit:contain; } .commit { justify-self:center; min-height:52px; margin:0; } .commit:disabled { opacity:.4; }
