<script lang="ts">
  ...
  onMount(() => {
    ...
    const unsubscribe = subscribeToRoom(ROOM_ID, (events) => { game = publicProjection(replay(events).state); ready = true; });
    return unsubscribe;
  });
...
<main class:busy aria-busy={busy} data-status={ready ? 'ready' : 'loading'} data-e2e-layout data-view={`${rotation}:${viewScale}:${panX}:${panY}`} style={`--starfield:url('${assets}/assets/starfield.webp');--view-rotation:${rotation}deg;--view-scale:${viewScale};--pan-x:${panX}px;--pan-y:${panY}px`}>
  ...
  <section class="edge far" aria-label="Imperial player edge">
    <div class="edge-status"><span class="seat-mark imperial"></span><strong>IMPERIAL EDGE</strong><span>{game.seats.imperial.joined ? 'PHONE LINKED' : 'PAIR PHONE'}</span></div>
  ...
  <aside class="rail right" aria-label="Public event log">
    <h2>FLIGHT LOG</h2>
    <ol>{#each game.log.slice(-6).reverse() as entry}<li>{entry}</li>{/each}</ol>
  </aside>

  <section class="edge near" aria-label="Rebel player edge">
    <div class="edge-status"><span class="seat-mark rebel"></span><strong>REBEL EDGE</strong><span>{game.seats.rebel.joined ? 'PHONE LINKED' : 'PAIR PHONE'}</span></div>
    {#if game.phase === 'lobby' && game.gameId !== 'uncreated'}
      <div class="pair-card">
